...
public class ScreenTransitionRenderer {
    ...
    public static void applyTransitionOut(ScreenTransition transition, float progress,
                                          GuiGraphics graphics, int screenWidth, int screenHeight,
                                          Easing easing) {
        PoseStack poseStack = graphics.pose();
        float easedProgress = easing.apply(progress);
        ...
        switch (transition) {
            case SLIDE_LEFT -> {
                float offset = easedProgress * -screenWidth;
                poseStack.translate(offset, 0, 0);
            }
            case SLIDE_RIGHT -> {
                float offset = easedProgress * screenWidth;
            ...
            case SLIDE_UP -> {
                float offset = easedProgress * -screenHeight;
                poseStack.translate(0, offset, 0);
            }
            case SLIDE_DOWN -> {
                float offset = easedProgress * screenHeight;
    ...
    public static void resetFade(GuiGraphics graphics) {
